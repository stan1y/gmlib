  ...
  _sticky(false)
{
  ...
  const texture & body = get_frame()->idle;
...
void push_btn::load(const data & d)
{
  ...
  set_idle_icon(resources::find(d["icon_idle"].as<std::string>()));

  if (d.has_key("icon_disabled"))
    set_disabled_icon(resources::find(d["icon_disabled"].as<std::string>()));

  if (d.has_key("icon_pressed"))
    set_pressed_icon(resources::find(d["icon_pressed"].as<std::string>()));

  if (d.has_key("sticky") && d["sticky"].as<bool>()) {
    set_sticky(true);
  }
}
...
{
  if (disabled()) {
    if(get_frame()->disabled.is_valid()) 
      get_frame()->disabled.render(r, dst);
